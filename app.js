...
// Initialize Supabase client — no auth persistence needed
...
function generateReport(period) {
  ...
  var f = function(v) { return sym + Math.abs(v).toLocaleString('en-IN', {minimumFractionDigits:2}); };
  var now = new Date();
  var startDate, periodLabel;
  if (period === 'week') {
    startDate = new Date(now); startDate.setDate(now.getDate() - 7);
    periodLabel = 'Weekly Report (' + startDate.toLocaleDateString('en-IN',{month:'short',day:'numeric'}) + ' - ' + now.toLocaleDateString('en-IN',{month:'short',day:'numeric',year:'numeric'}) + ')';
  } else {
    startDate = new Date(now.getFullYear(), now.getMonth(), 1);
    periodLabel = 'Monthly Report - ' + now.toLocaleDateString('en-IN',{month:'long',year:'numeric'});
  }
  var startStr = startDate.toISOString().split('T')[0];
  var txns = state.transactions.filter(function(t) { return t.date >= startStr; });
  ...
  var totalExpenses = expenses.reduce(function(s,t) { return s + t.amount; }, 0);
  var netSavings = totalIncome - totalExpenses;
  var savingsRate = totalIncome > 0 ? ((netSavings / totalIncome) * 100).toFixed(1) : '0.0';
  var catMap = {};
  expenses.forEach(function(t) { catMap[t.category] = (catMap[t.category] || 0) + t.amount; });
  var categories = Object.entries(catMap).sort(function(a,b) { return b[1] - a[1]; });
  var budgetLines = state.budgets.map(function(b) {
    var spent = catMap[b.category] || 0;
    var pct = ((spent / b.limit) * 100).toFixed(0);
    var status = spent > b.limit ? 'OVER LIMIT' : spent > b.limit * 0.7 ? 'WARNING' : 'OK';
    return '   ' + b.category + ': ' + f(spent) + ' / ' + f(b.limit) + ' (' + pct + '%) - ' + status;
  });
  var topExp = expenses.slice().sort(function(a,b) { return b.amount - a.amount; }).slice(0, 5);
  var topLines = topExp.map(function(t,i) { return '   ' + (i+1) + '. ' + t.description + ' - ' + f(t.amount) + ' (' + t.category + ', ' + formatDate(t.date) + ')'; });
  var r = 'FINFLOW ' + periodLabel.toUpperCase() + '\n';
  r += '================================\n\n';
  r += 'FINANCIAL SUMMARY\n';
  r += '   Total Income:     ' + f(totalIncome) + '\n';
  r += '   Total Expenses:   ' + f(totalExpenses) + '\n';
  r += '   Net Savings:      ' + f(netSavings) + (netSavings >= 0 ? ' (Positive)' : ' (Negative)') + '\n';
  r += '   Savings Rate:     ' + savingsRate + '%\n';
  r += '   Transactions:     ' + txns.length + ' total (' + income.length + ' income, ' + expenses.length + ' expenses)\n\n';
  if (categories.length > 0) {
    r += 'EXPENSE BREAKDOWN BY CATEGORY\n';
    categories.forEach(function(cat) { r += '   ' + cat[0] + ': ' + f(cat[1]) + ' (' + ((cat[1]/totalExpenses)*100).toFixed(1) + '%)\n'; });
    r += '\n';
  }
  if (budgetLines.length > 0) { r += 'BUDGET STATUS\n' + budgetLines.join('\n') + '\n\n'; }
  if (topLines.length > 0) { r += 'TOP EXPENSES\n' + topLines.join('\n') + '\n\n'; }
  r += '================================\n';
  r += 'Generated by FinFlow on ' + now.toLocaleDateString('en-IN',{weekday:'long',year:'numeric',month:'long',day:'numeric'}) + ' at ' + now.toLocaleTimeString('en-IN') + '\n';
  return { report: r, periodLabel: periodLabel, totalIncome: totalIncome, totalExpenses: totalExpenses, netSavings: netSavings, savingsRate: savingsRate };
}

async function sendTestAlert() {
  var key = document.getElementById('settings-web3forms-key').value.trim();
  var email = document.getElementById('settings-alert-email').value.trim();
  if (!key) { showToast('Enter your Web3Forms key first.', 'error'); return; }
  if (!email) { showToast('Enter alert email first.', 'error'); return; }
  showToast('Generating test report...', 'info');
  try {
    var rpt = generateReport('week');
    var subject = 'FinFlow Test Report | Income: ' + getCurrencySymbol() + rpt.totalIncome.toLocaleString('en-IN') + ' | Savings: ' + rpt.savingsRate + '%';
    var res = await fetch('https://api.web3forms.com/submit', {
      method: 'POST', headers: {'Content-Type':'application/json','Accept':'application/json'},
      body: JSON.stringify({ access_key: key, subject: subject, from_name: 'FinFlow Reports', to: email, message: rpt.report })
    });
    var rr = await res.json();
    if (rr.success) showToast('Test report sent! Check your inbox.', 'success');
    else showToast('Failed: ' + rr.message, 'error');
  } catch (err) { showToast('Failed to send report.', 'error'); }
}

async function sendWeeklyReport() {
  var key = document.getElementById('settings-web3forms-key').value.trim() || state.settings.web3formsKey;
  var email = document.getElementById('settings-alert-email').value.trim() || state.settings.alertEmail;
  if (!key) { showToast('Set your Web3Forms key in Email Alerts settings.', 'error'); return; }
  if (!email) { showToast('Set your alert email in settings.', 'error'); return; }
  showToast('Generating weekly report...', 'info');
  try {
    var rpt = generateReport('week');
    var subject = 'FinFlow Weekly Report | Income: ' + getCurrencySymbol() + rpt.totalIncome.toLocaleString('en-IN') + ' | Expenses: ' + getCurrencySymbol() + rpt.totalExpenses.toLocaleString('en-IN');
    var res = await fetch('https://api.web3forms.com/submit', {
      method: 'POST', headers: {'Content-Type':'application/json','Accept':'application/json'},
      body: JSON.stringify({ access_key: key, subject: subject, from_name: 'FinFlow Weekly Report', to: email, message: rpt.report })
    });
    var rr = await res.json();
    if (rr.success) showToast('Weekly report sent! Check your inbox.', 'success');
    else showToast('Failed: ' + rr.message, 'error');
  } catch (err) { showToast('Failed to send weekly report.', 'error'); }
}
